...
async function initializeDatabase() {
    ...
    const serverPool = new pg_1.Pool({
        connectionString: pgConnectionString,
        connectionTimeoutMillis: 5000,
    });
    try {
        // Check if database exists
        const dbCheckResult = await serverPool.query(`SELECT 1 FROM pg_database WHERE datname = $1`, [dbName]);
        ...
        const tableCheckResult = await dbPool.query(`SELECT table_name FROM information_schema.tables 
